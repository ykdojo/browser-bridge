...
- `extension/` - Manifest V3, thin relay. Connects to `ws://127.0.0.1:17333`, forwards Chrome DevTools Protocol (CDP) commands to tabs through `chrome.debugger`, buffers console/network events per tab, and answers JavaScript dialogs the agent triggers. Its toolbar popup shows connection status and links to a short "How it works" page. Pings every 20s to keep the service worker alive, retries every 30s via `chrome.alarms` when the server is not running.
...
- **Mouse input needs a visible tab.** Chrome stops drawing hidden tabs and mouse input waits on drawing (measured: a click takes 5s, a scroll never returns). So mouse actions first make the tab the visible one in its window, without taking OS focus, and say so in the result. Reading, typing and screenshots work on hidden tabs as they are. To browse alongside an agent undisturbed, give it its own window.
- **Multiple Chrome profiles.** Each profile with the extension connects; the most recently connected one is active, and the toolbar popup in another profile can switch to it. The popup also shows whether a server is connected, and the version.
...
Known gaps: while no agent session is running, the extension's card on `chrome://extensions` shows a `net::ERR_CONNECTION_REFUSED` error. That is the extension looking for a server that isn't there; Chrome logs it itself and code can't silence it. It does not mean anything is broken. No per-origin allowlist or confirmations yet. `read_page` and `find` don't see inside iframes (coordinate clicks do reach them). Sessions share tabs with no locking between them.
