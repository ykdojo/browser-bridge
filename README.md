...
- `extension/` - MV3, thin relay. Connects to `ws://127.0.0.1:17333`, forwards CDP commands to tabs through `chrome.debugger`, and buffers console/network events per tab. Pings every 20s to keep the service worker alive, retries every 30s via `chrome.alarms` when the server is not running.
