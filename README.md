...
- **Looking for the server quietly.** With no agent session running there is nothing to connect to, and Chrome prints an error on `chrome://extensions` for every refused WebSocket that code can't catch. A refused `fetch` prints nothing, so the extension asks the port who is there with `fetch` every 2s and opens the WebSocket only once a Browser Bridge server answers.
